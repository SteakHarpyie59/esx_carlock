				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Du hast deinen ~y~'..vehicleLabel..'~s~ ~r~abgeschlossen~s~.')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Du hast deinen ~y~'..vehicleLabel..'~s~ ~g~aufgeschlossen~s~.')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
						...
						ESX.TriggerServerCallback("sh59_KeySystem:CheckIfShared", function(cb) 
							if cb and hasAlreadyLocked ~= true then
								...
								if lock == 1 or lock == 0 then
									...
									SetVehicleDoorsLocked(carstrie[i], 2)
									PlayVehicleDoorCloseSound(carstrie[i], 1)
									ESX.ShowNotification('Du hast den ~y~'..vehicleLabel..'~s~ ~r~abgeschlossen~s~.')
									if not IsPedInAnyVehicle(PlayerPedId(), true) then
										TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
									end
									SetVehicleLights(carstrie[i], 2)
								...
								elseif lock == 2 then
									SetVehicleDoorsLocked(carstrie[i], 1)
									PlayVehicleDoorOpenSound(carstrie[i], 0)
									ESX.ShowNotification('Du hast den ~y~'..vehicleLabel..'~s~ ~g~aufgeschlossen~s~.')
									if not IsPedInAnyVehicle(PlayerPedId(), true) then
										TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
									end
									SetVehicleLights(carstrie[i], 2)
